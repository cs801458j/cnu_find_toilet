    ...
    gps(){
      ...
      if(navigator.geolocation){//gps 사용
        ...
        navigator.geolocation.getCurrentPosition(function(position){
          ...
          var locPosition = this.maps.LatLng(lat, lon),
              message = '<div style="padding:5px;">현재 위치</div>';
          displayMarker(locPosition, message);
        });
      }else{//gps 사용불가
        var locPosition = this.maps.LatLng(36.36662192460574, 127.34445497915917),
    ...
    searchToilet: function(){
            ...
            .then((result) => {
               for(var i=0; i<result.data.resultlist.length; i++){
                this.addressTogeo(result.data.resultlist[i].locplc_lnm)
              }
            }) },
